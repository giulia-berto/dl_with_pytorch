...
if [ ! -f all.json ]; then
    echo "Retrieving IDs..."
    bl dataset query --limit 10000 -p $projID --datatype $datatype --tag $tag --tag --json > all.json
fi
...
for subjID in $(jq -r '.[].meta.subject' all.json | sort -u); do
    ...
    do
        ...
        out_fname=$out_dir/sub-${subjID}_space-mni152_dwi.nii.gz
        mv $id/dwi.nii.gz $out_fname
